...
static int write_to_file(const char *path, const char *value)
{
    ...
    if (!fp) {
        ...
        snprintf(logBuf, sizeof(logBuf),
                 "write_to_file: fopen failed path=%s value=%s errno=%d (%s)",
                 path, value, errno, strerror(errno));
        LOGE("BOOST_MANAGER", logBuf);
        return -1;
    }
    if (fputs(value, fp) == EOF) {
        char logBuf[512];
        snprintf(logBuf, sizeof(logBuf),
                 "write_to_file: fputs failed path=%s value=%s errno=%d (%s)",
                 path, value, errno, strerror(errno));
        LOGE("BOOST_MANAGER", logBuf);
        fclose(fp);
        return -1;
    ...
    fclose(fp);
    {
        char logBuf[512];
        snprintf(logBuf, sizeof(logBuf),
                 "write_to_file: wrote path=%s value=%s", path, value);
        LOGE("BOOST_MANAGER", logBuf);
    }
    return 0;
}
...
static int move_tid_to_cgroup(pid_t tid, const char *cg_path)
{
    ...
    int ret = write_to_file(cgroup_path, tid_str);
    if (ret != 0) {
        char logBuf[512];
        snprintf(logBuf, sizeof(logBuf),
                 "move_tid_to_cgroup: failed tid=%d target_cg=%s target_file=%s errno=%d (%s)",
                 (int)tid, cg_path, cgroup_path, errno, strerror(errno));
        LOGE("BOOST_MANAGER", logBuf);
    } else {
        char logBuf[512];
        snprintf(logBuf, sizeof(logBuf),
                 "move_tid_to_cgroup: moved tid=%d target_cg=%s target_file=%s",
                 (int)tid, cg_path, cgroup_path);
        LOGE("BOOST_MANAGER", logBuf);
    }
    return ret;
}
...
static int pin_tid_to_core(pid_t tid, int core)
{
    ...
    if (ret != 0) {
        char logBuf[512];
        snprintf(logBuf, sizeof(logBuf),
                 "pin_tid_to_core: sched_setaffinity failed tid=%d core=%d errno=%d (%s)",
                 (int)tid, core, errno, strerror(errno));
        LOGE("BOOST_MANAGER", logBuf);
        return ret;
    }
    ...
    cpu_set_t verify;
    CPU_ZERO(&verify);
    if (sched_getaffinity(tid, sizeof(verify), &verify) != 0) {
        char logBuf[512];
        snprintf(logBuf, sizeof(logBuf),
                 "pin_tid_to_core: sched_getaffinity failed tid=%d core=%d errno=%d (%s)",
                 (int)tid, core, errno, strerror(errno));
        LOGE("BOOST_MANAGER", logBuf);
        return ret;
    }
    ...
    if (!CPU_ISSET(core, &verify)) {
        char logBuf[256];
        snprintf(logBuf, sizeof(logBuf),
                 "pin_tid_to_core: verification failed tid=%d requested_core=%d",
                 (int)tid, core);
        LOGE("BOOST_MANAGER", logBuf);
    } else {
        char logBuf[256];
        snprintf(logBuf, sizeof(logBuf),
                 "pin_tid_to_core: pinned tid=%d core=%d",
                 (int)tid, core);
        LOGE("BOOST_MANAGER", logBuf);
    }
...
void boostManagerApply(BoostManagerCtx *ctx,
    ...
    if (!ctx->isolation_active) {
        ...
        if (set_cgroup_cpuset(cfg->boostCgroupPath, boost_cpuset) != 0) {
            char logBuf[512];
            snprintf(logBuf, sizeof(logBuf),
                     "boostManagerApply: failed to set boost cpuset cg=%s cpus=%s errno=%d (%s)",
                     cfg->boostCgroupPath, boost_cpuset, errno, strerror(errno));
            LOGE("BOOST_MANAGER", logBuf);
        }
        ...
        if (set_cgroup_cpuset(cfg->focusedCgroupPath, focused_cpuset) != 0) {
            char logBuf[512];
            snprintf(logBuf, sizeof(logBuf),
                     "boostManagerApply: failed to set focused cpuset cg=%s cpus=%s errno=%d (%s)",
                     cfg->focusedCgroupPath, focused_cpuset, errno, strerror(errno));
            LOGE("BOOST_MANAGER", logBuf);
        }
    ...
    int moved_count = 0;
    for (int i = 0; i < result->hot_count; i++) {
        if (move_tid_to_cgroup(result->hot_tids[i], cfg->boostCgroupPath) == 0) {
            ctx->boosted_tids[moved_count] = result->hot_tids[i];
            moved_count++;
        }
    }
    ctx->boosted_count = moved_count;

    if (ctx->boosted_count != result->hot_count) {
        char logBuf[256];
        snprintf(logBuf, sizeof(logBuf),
                 "boostManagerApply: not all hot tids moved requested=%d moved=%d",
                 result->hot_count, ctx->boosted_count);
        LOGE("BOOST_MANAGER", logBuf);
        return;
    }
    ...
    if (result->type == APP_ST) {
        /* ST: rotate across favored cores using the same rotation state as DT. */
        if (cfg->dt_favored_core_count >= 1) {
            int phase = ctx->dt_rotation_phase;
            int core = cfg->favoredCoresDT[phase % cfg->dt_favored_core_count];
            char logBuf[256];
            snprintf(logBuf, sizeof(logBuf),
                     "boostManagerApply: ST pin tid=%d favored_core=%d",
                     (int)ctx->boosted_tids[0], core);
            LOGE("BOOST_MANAGER", logBuf);
            pin_tid_to_core(ctx->boosted_tids[0], core);
        }
        if (tids_changed)
            ctx->dt_rotation_counter_ms = 0;
...
void boost_manager_rotate_tick(BoostManagerCtx *ctx) {
    ...
    /* Rotation is only for ST. DT keeps fixed placement on favored cores. */
    if (ctx->boosted_count != 1 || cfg->dt_favored_core_count <= 0)
        return;
