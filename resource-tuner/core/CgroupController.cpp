 ...
 */
static int read_schedstat_file(const char *path, uint64_t *run_ns)
{
...
static int read_thread_schedstat(pid_t tid, pid_t pid, uint64_t *run_ns)
{
    ...
    return read_schedstat_file(path, run_ns);
}

static int read_tid_schedstat(pid_t tid, uint64_t *run_ns)
{
    char path[128];
    snprintf(path, sizeof(path), "/proc/%d/schedstat", (int)tid);
    return read_schedstat_file(path, run_ns);
}

static int add_thread_sample(struct thread_sample *out, int count, int max_threads,
                             pid_t tid, uint64_t run_ns)
{
    ...
    out[count].tid = tid;
    out[count].run_ns = run_ns;
    return count + 1;
}

/**
 * Snapshot all threads in the given cgroup using schedstat.
 * Prefer cgroup.threads because threaded cgroups may not allow reading cgroup.procs.
 */
static int snapshot_cgroup_threads_append(const char *cg_path,
                                          struct thread_sample *out,
                                          int max_threads,
                                          int count)
{
    char threads_path[256];
    snprintf(threads_path, sizeof(threads_path), "%s/cgroup.threads", cg_path);

    FILE *tfp = fopen(threads_path, "r");
    if (tfp) {
        char line[64];
        while (fgets(line, sizeof(line), tfp) && count < max_threads) {
            pid_t tid = (pid_t)std::atoi(line);
            uint64_t rns = 0;
            if (tid > 0 && read_tid_schedstat(tid, &rns) == 0)
                count = add_thread_sample(out, count, max_threads, tid, rns);
        }
        fclose(tfp);
        return count;
    }
...
static int snapshot_cgroup_threads(const char *cg_path,
                                   ...
                                   int max_threads)
{
    return snapshot_cgroup_threads_append(cg_path, out, max_threads, 0);
}

static int snapshot_two_cgroups_threads(const char *cg_path_a,
                                        const char *cg_path_b,
                                        struct thread_sample *out,
                                        int max_threads)
{
    int count = snapshot_cgroup_threads_append(cg_path_a, out, max_threads, 0);
    return snapshot_cgroup_threads_append(cg_path_b, out, max_threads, count);
}
...
static enum WorkloadType classifySamples(AppTrackerCtx *ctx,
                                          struct thread_sample *curr,
                                          int curr_count,
...
enum WorkloadType classifyCgroup(AppTrackerCtx *ctx,
    ...
    return classifySamples(ctx, curr, curr_count, result);
}

enum WorkloadType classifyCgroups(AppTrackerCtx *ctx,
                                  const char *cg_path_a,
                                  const char *cg_path_b,
                                  struct classify_result *result) {
    struct thread_sample curr[MAX_THREADS];
    int curr_count = snapshot_two_cgroups_threads(cg_path_a, cg_path_b, curr, MAX_THREADS);
    return classifySamples(ctx, curr, curr_count, result);
}
