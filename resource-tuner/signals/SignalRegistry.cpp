...
SignalInfo* SignalRegistry::findBestExtraAttrsMatch(DLManager* dlm, const uint32_t* extraAttrs) const {
    if(dlm == nullptr || extraAttrs == nullptr) {
        return nullptr;
    }
    ...
    DL_ITERATE(dlm) {
        SigIterable* iterable = (SigIterable*) iter;
        if(iterable == nullptr || iterable->mData == nullptr) {
            continue;
        }

        SignalInfo* candidate = (SignalInfo*) iterable->mData;
...
void SignalRegistry::registerSignal(SignalInfo* signalInfo) {
    ...
    if(this->mSignalsConfigs.find(signalBitmap) == this->mSignalsConfigs.end()) {
        this->mSignalsConfigs[signalBitmap].signalInfo = nullptr;
        this->mSignalsConfigs[signalBitmap].signalListMgr = nullptr;

        if(signalInfo->mHasExtraAttrs) {
            this->mSignalsConfigs[signalBitmap].signalListMgr = new DLManager(0);
        }
    } else {
        if(!signalInfo->mHasExtraAttrs) {
            isOverride = true;
            freeSignalConfig(this->mSignalsConfigs[signalBitmap].signalInfo);
            this->mSignalsConfigs[signalBitmap].signalInfo = nullptr;
        }
    }

    if(signalInfo->mHasExtraAttrs) {
        SigIterable* iterable = new SigIterable;
        iterable->mData = signalInfo;
        this->mSignalsConfigs[signalBitmap].signalListMgr->insert(iterable);
    } else {
        this->mSignalsConfigs[signalBitmap].signalInfo = signalInfo;
    }
...
SignalInfo* SignalRegistry::getSignalConfigById(uint64_t sigCode,
    ...
    if(extraAttrs != nullptr) {
        DLManager* dlm = this->mSignalsConfigs[sigCode].signalListMgr;
        if(dlm == nullptr) {
            return nullptr;
        }
        return findBestExtraAttrsMatch(dlm, extraAttrs);
    }

    return this->mSignalsConfigs[sigCode].signalInfo;
}
...
SignalInfo* SignalRegistry::getSignalConfigById(uint32_t sigId,
    ...
    if(extraAttrs != nullptr) {
        DLManager* dlm = this->mSignalsConfigs[signalBitmap].signalListMgr;
        if(dlm == nullptr) {
            return nullptr;
        }
        return findBestExtraAttrsMatch(dlm, extraAttrs);
    ...
    return this->mSignalsConfigs[signalBitmap].signalInfo;
}
...
SignalRegistry::~SignalRegistry() {
    for(std::pair<uint64_t, SignalConf> entry: this->mSignalsConfigs) {
        if(entry.second.signalListMgr != nullptr) {
            DLRootNode* iter = entry.second.signalListMgr->mHead;
            while(iter != nullptr) {
                DLRootNode* next = iter->getNextPtr(entry.second.signalListMgr->mLinkerInUse);
                SigIterable* iterable = (SigIterable*) iter;
                if(iterable->mData != nullptr) {
                    freeSignalConfig(iterable->mData);
                    iterable->mData = nullptr;
                }
                delete iterable;
                iter = next;
            }
            delete entry.second.signalListMgr;
            entry.second.signalListMgr = nullptr;
        }

        if(entry.second.signalInfo != nullptr) {
            freeSignalConfig(entry.second.signalInfo);
            entry.second.signalInfo = nullptr;
        }
    }
}
